...
export async function GET(request: NextRequest) {
  ...
  const existPks = existSourceIds.filter((note) => note.sourceId).map((note) => note.sourceId)
...
async function sync(uid: number, lang: string, hash: { [key: string]: Lingq }, pks: string[]) {
  if (pks.length === 0) {
    return
  }
