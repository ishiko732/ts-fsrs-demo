...
async function syncUser() {
    const users = await prisma.parameters.findMany({
      where: {
        lingq_token: {
          not: null,
        },
...
async function syncLingqs(user: DecryptSyncUser, lang: languageCode, next?: number) {
    const data = await getLingqs({ language: lang, token: user.token, page: next, page_size: 50,search_criteria:'startsWith',sort:'date',status:['0','1','2','3'] })
