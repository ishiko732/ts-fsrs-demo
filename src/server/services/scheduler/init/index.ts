...
import progeigo from '@/../public/プログラミング必須英単語600+.json' with {
  type: 'json',
}
...
// init progeigo dates
export async function initProgeigoDates(
  ...
  const notes = await trx
    .insertInto('notes')
    .values(datum)
    .returning(['uid', 'did', 'id', 'created'])
    .execute()
  const card_for_fsrs = createEmptyCard(now)

  const cards = notes.map((note) =>
    initCard(note.uid, note.did, note.id, card_for_fsrs)
  )
  await trx.insertInto('cards').values(cards).execute()
}

function initProgeigoNote(
  uid: number,
  did: number,
  data: ProgeigoNodeData,
  now: Date
): Insertable<NoteTable> | null {
...
export function initCard(
  ...
  return {
    ...
    ...card,
    due: +card.due,
    created: now ? Date.now() : undefined,
    updated: now ? Date.now() : undefined,
