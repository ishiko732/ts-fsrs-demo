...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('created', 'bigint', (col) =>
      col.defaultTo(sql`extract(epoch from now()) * 1000`).notNull()
    )
    .addColumn('updated', 'bigint', (col) =>
      col.defaultTo(sql`extract(epoch from now()) * 1000`).notNull()
    )
    .execute()
  ...
  await db.schema
    ...
    .execute()
  await db.schema
    .createIndex('users_oauth_index')
    .on('users')
    .columns(['oauthId', 'oauthType'])
    .unique()
    .execute()
  await db.schema
    .createIndex('users_created_index')
    .on('users')
    .columns(['created'])
    .execute()
  ...
  await db.schema
    .createIndex('decks_uid_index')
    .on('decks')
    .columns(['uid'])
    .execute()
  await db.schema
    .createIndex('decks_created_index')
    .on('decks')
    .columns(['created'])
    .execute()
  ...
  await db.schema
    .createIndex('notes_uid_index')
    .on('notes')
    .columns(['uid'])
    .execute()
  await db.schema
    .createIndex('notes_did_index')
    .on('notes')
    .columns(['did'])
    .execute()
  ...
  await db.schema
    .createIndex('cards_uid_did_index')
    .on('cards')
    .columns(['uid', 'did'])
    .execute()
  ...
  await db.schema
    .createIndex('revlog_uid_index')
    .on('revlog')
    .columns(['uid'])
    .execute()
  await db.schema
    .createIndex('revlog_did_index')
    .on('revlog')
    .columns(['uid', 'did'])
    .execute()
  await db.schema
    .createIndex('revlog_cid_index')
    .on('revlog')
    .columns(['uid', 'cid'])
    .execute()
  ...
  await db.schema
    .createIndex('extra_uid_index')
    .on('extras')
    .columns(['uid'])
    .execute()
  await db.schema
    .createIndex('extra_did_index')
    .on('extras')
    .columns(['uid', 'did'])
    .execute()
}
