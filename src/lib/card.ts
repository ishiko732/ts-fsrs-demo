...
export function schedulerCard<T extends Grade>(
  query: Partial<Query>,
  now: Date,
  grade?: Grade,
): Promise<T extends Grade ? RepeatRecordLog : RecordLogPrisma>

export async function schedulerCard<T extends Grade>(
  query: Partial<Query>,
  now: Date,
  grade?: Grade,
): Promise<T extends Grade ? RepeatRecordLog : RecordLogPrisma> {
...
export async function updateCard(cid: number, now: Date, grade: Grade, duration: number) {
  const recordItem = await schedulerCard({ cid }, now, Number(grade) as Grade)
...
export async function rollbackCard(query: Partial<Query>) {
  ...
  return getNoteByCid(cardByPrisma.cid)
}
